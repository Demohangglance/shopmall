  ...
  <div>
    ...
    <el-card>
      ...
      <el-table :data="tableData" stripe>
        ...
        <el-table-column align="center" prop="goodsid" label="商品" :show-overflow-tooltip="true">
        ...
        <el-table-column align="center" prop="ordersid" label="订单编号" :show-overflow-tooltip="true">
          <template v-slot="scope">
            {{ orders.find(v => v.id == scope.row.ordersid)?.orderno}}
          </template>
        </el-table-column>
        <el-table-column align="center" prop="comment" label="评价" :show-overflow-tooltip="true"></el-table-column>
    ...
    <el-dialog title="信息" :visible.sync="dialogFormVisible" width="30%" :close-on-click-modal="false">
      <el-form label-width="100px" style="padding-right: 40px" :model="form" :rules="rules" ref="ruleForm">
        ...
        <el-form-item prop="ordersid" label="订单ID">
          <el-input v-model="form.ordersid" autocomplete="off"></el-input>
        </el-form-item>
      ...
      <div class="drawer-content" ref="drawerContent">
        <el-form label-width="100px" style="padding-right: 40px" :model="form">
          <el-form-item prop="goodsid" label="商品">
            <div>{{ goods.find(v => v.id == form.goodsid)?.name}}</div>
          </el-form-item>
          <el-form-item prop="num" label="数量">
            <div>{{form.num}}</div>
          </el-form-item>
          ...
          <el-form-item prop="ordersid" label="订单编号">
            <div>{{ orders.find(v => v.id == form.ordersid)?.orderno}}</div>
          </el-form-item>
          <el-form-item prop="comment" label="评价">
          ...
          <el-form-item prop="score" label="评分">
            <div>
              <el-rate style="margin-top: 7px" v-model="form.score" disabled></el-rate>
            </div>
          </el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  methods: {
    load() {
      this.$request.get("/ordersitem/page", {
        params: {
      ...
      this.$request.get("/orders").then(res => {
    ...
    del(id) {
      this.$confirm('您确认删除这些数据吗？', '确认删除', {type: "warning"}).then(response => {
       this.$request.delete("/ordersitem/" + id).then(res => {
          if (res.code === '200') {
            this.$notify.success({title: '成功', message: '操作成功', showClose: false, duration: 2000});
            this.load()
          } else {
